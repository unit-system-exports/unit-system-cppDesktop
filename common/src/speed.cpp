...
base::length common::operator*(const common::speed& v, const base::time& t){
    auto v1 = base::unit_cast(v,1);
    auto t1 = base::unit_cast(t,1);
    return base::length{v1.value*t1.value,1};
}

base::length common::operator*(const base::time& t, const common::speed& v){
    return v*t;
}

base::time operator/(const base::length& s, const common::speed& v){
    auto v1 = base::unit_cast(v,1);
    auto s1 = base::unit_cast(s,1);
    return base::time{s1.value/v1.value,1};
}
